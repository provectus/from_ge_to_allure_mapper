...
def get_json(ge_root_dir,test_suite):
    file = glob.glob(os.path.join(os.path.abspath(ge_root_dir), 'great_expectations/uncommitted/validations/'+test_suite+'/**/**/*.json'), recursive=False)
...
def count_of_suits():
    file = get_json()
    count_of_tests = len(file['results'])
    count_of_table_tests = 0
    count_of_columns_in_tests = 0
    columns = []

    for result in file['results']:
        if 'column' not in result['expectation_config']['kwargs']:
            count_of_table_tests += 1
        if 'column' in result['expectation_config']['kwargs']:
            columns.append(result['expectation_config']['kwargs']['column'])

    count_of_columns_in_tests = len(set(columns))
    return 1 + count_of_columns_in_tests  if count_of_table_tests!=0 else count_of_columns_in_tests
...
# TODO learn how to know
def get_start_suit_time():
    return 1480450640411
...
def get_observed_value(file):
    return "Observed value: "+str(file['result']['observed_value']) if 'observed_value' in file['result'] else "Unexcpected count: "+str(file['result']['unexpected_count'])
...
def create_suit_json(allure_result,allure_report,ge_root_dir,test_suite):
    ...
    # if os.path.exists('dags/reportsx'):
    if os.path.exists(allure_result):
        # shutil.rmtree('dags/reportsx/')
        shutil.rmtree(allure_result+'/')
    os.makedirs(allure_result)
    # os.makedirs('dags/reportsx')
    ...
    for i in file['results']:
        ...
        data = {
                ...
                "start": get_start_suit_time(),
                "stop": get_stop_suit_time(),
                "steps": [
            {
                "status": get_test_status(i),
